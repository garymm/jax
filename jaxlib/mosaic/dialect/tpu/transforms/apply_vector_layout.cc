...
FailureOr<std::pair<Value, SmallVector<int64_t>>> sliceRef(
  ...
  for (auto [tiled_idx, t] :
       llvm::zip_equal(indices.take_back(tiling.size()), tiling)) {
    if (auto cst = getIntConst(tiled_idx, /*silent=*/true); succeeded(cst)) {
      ...
      const std::optional<int64_t> offset = getKnownModulo(tiled_idx, t);
      if (!offset) {
        return failure();
      }
      indices_within_slice.push_back(*offset);
      auto t_const =
          builder.create<arith::ConstantOp>(i32, builder.getI32IntegerAttr(t));
      // TODO(tlongeri): Aligning to tiling with div+mul seems wasteful since
      // when lowering we will just do integer division by tiling. Perhaps we
      // can just say MemrefSlice ignores within-tile offsets. Would need to
      // remove checks and update debug assert insertion pass.
      slice_base_indices.push_back(builder.create<arith::MulIOp>(
          builder.create<arith::DivUIOp>(
              builder.create<arith::IndexCastOp>(i32, tiled_idx), t_const),
          t_const));
    }
  }
